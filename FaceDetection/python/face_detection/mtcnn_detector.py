...
from .helper import nms, adjust_input, detect_first_stage_wrapper
...
class MtcnnDetector(object):
    ...
    def __init__(self,
                 model_folder='.',
                 minsize=20,  # 待检测图像的最小尺寸
                 threshold=[0.6, 0.7, 0.8],  # 三个阶段的网络阈值
                 factor=0.709,
                 num_worker=1,
    ...
    def detect_face(self, img):
        ...
        # 计算图像金字塔各层的缩放尺寸
        scales = []
        m = MIN_DET_SIZE / self.minsize  # 12/20
        minl *= m
        factor_count = 0
        ...
        #     # 每批都并行处理num_worker个金字塔图像
        #     # 同步阻塞
        #     local_boxes = self.Pool.map(detect_first_stage_wrapper,
        #                                 zip(repeat(img), self.PNets[:len(batch)], [scales[i] for i in batch],
        #                                     repeat(self.threshold[0])))
        #     total_boxes.extend(local_boxes)

        for i in range(len(scales)):
            local_boxes = detect_first_stage_wrapper([img, self.PNet, scales[i], self.threshold[0]])
        ...
        # merge the detection from first stage 再一次非极大抑制
        ...
        # refine the bboxes
        # todo:bounding box regression校正
        # x_new=x_old+w_x*width
        # y_new=y_old+w_y*height
        ...
        total_boxes = self.convert_to_square(total_boxes)  # 将每个BBox resize成方形
    ...
    def extract_image_chips(self, img, points, desired_size=256, padding=0):
        ...
        for p in points:
            ...
            for k in range(int(len(p) / 2)):
                shape.append(p[k])
                shape.append(p[k + 5])  # x1,y1,x2,y2...
